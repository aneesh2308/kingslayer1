...
const NewBooksList = ({ navigation }) => {
  ...
  const getData = async () => {
    ...
    fetch(`${apiURL}/books`, requestOptions)
      ...
      .then((result) => {
        var temp=result.filter(x=>x.bookCondition=="New")
        console.log("new book result",temp);
        setData(temp);
        setExtraData(temp);
        setInitialData(temp);
      })
      .catch((error) => console.log("new book error", error));
  };
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => {
          console.log(item.bookimg!=undefined?item.bookimg[0]:item.img);
...
export default inject("store")(observer(NewBooksList));
