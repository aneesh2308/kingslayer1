...
const EbooksList = ({ navigation }) => {
  ...
  const getData = async () => {
    ...
    fetch(`${apiURL}/books`, requestOptions)
      ...
      .then((result) => {
        var temp=result.filter(x=>x.bookRef=="Ebook")
        console.log("ebook book result",temp);
        setData(temp);
        setExtraData(temp);
        setInitialData(temp);
      })
      .catch((error) => console.log("ebook error", error));
  };
...
export default inject("store")(observer(EbooksList));
